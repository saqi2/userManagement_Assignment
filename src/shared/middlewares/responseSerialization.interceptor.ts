...
export class ResponseSerialization implements NestInterceptor {
  ...
  public intercept (_context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      map(resBody => {
        const temp = JSON.stringify(resBody);
        resBody = JSON.parse(temp.replace(/_id/g, 'id'));
        ...
        return { data: resBody.dataValues ? resBody.dataValues : resBody, count: resBody?.count };
      }),
    );
  }
}
